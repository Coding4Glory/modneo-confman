...
describe('check pretest conditions ', function()
    it('fixture exists', function()
        assert.truthy(vim.fn.isdirectory(vim.fs.joinpath(uv.cwd(), fixture_settings.plugin_lib)))
        assert.truthy(vim.fn.isdirectory(vim.fs.joinpath(uv.cwd(), fixture_settings.plugin_lib, 'cat_one')))
        assert.truthy(vim.fn.isdirectory(vim.fs.joinpath(uv.cwd(), fixture_settings.plugin_lib, 'cat_two')))
        assert.truthy(vim.fn.isdirectory(vim.fs.joinpath(uv.cwd(), fixture_settings.plugin_lib, 'enabled')))
    end)
...
describe('core module ', function()
    describe('test pattern building:', function()
        ...
        it('without name', function()
            ...
            assert.truthy(vim.endswith(pattern, config.options.default_filter))
        end)
    end)
